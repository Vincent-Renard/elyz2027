...
export default function ProgramSection({ candidateId, programUrl }: { candidateId: string; programUrl?: string }) {
  const [query, setQuery] = useState("");

  const points = programs[candidateId] ?? [];
  const filtered = useMemo(() => {
    if (!query.trim()) return points;
    const lower = query.toLowerCase();
    return points.filter((p) => p.toLowerCase().includes(lower));
  }, [query, points]);

  if (points.length === 0 && !programUrl) {
    return (
      <div className="mt-8">
    ...
    <div className="mt-8">
      ...
      <ul className="mt-3 space-y-2">
        ...
        {filtered.length === 0 && query && (
          <li className="rounded-lg border border-zinc-100 bg-zinc-50 px-4 py-3 text-sm text-zinc-400 italic">
