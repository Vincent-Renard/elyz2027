...
function EventIcon({ type, candidateId }: { type: string; candidateId?: string }) {
  if (type === "election") {
    return (
      <div className="flex h-10 w-10 items-center justify-center rounded-full bg-elyz-blue text-white ring-4 ring-white">
  ...
  return (
    <div
      className="flex h-10 w-10 items-center justify-center rounded-full text-white ring-4 ring-white"
        ...
        {timelineEvents.map((event) => (
          <li key={event.id} className="relative flex gap-6">
            <div className="relative z-10 flex-shrink-0">
              <EventIcon type={event.type} candidateId={event.candidateId} />
            </div>
