...
export default function EvolutionChart() {
  ...
  const labelW = 170;
  const rowH = 56;
  const chartH = sorted.length * rowH + 50;
            ...
            return (
              <g key={c.name}>
                {c.scores.map((s) => {
                  ...
                  if (pi === -1) return null;
                  const x = (pi / (polls.length - 1 || 1)) * (chartW - labelW) + labelW;
                  dots.push(x);
                  return (
                    <g key={s.pollId}>
                ...
                <circle cx={10} cy={y} r={4} fill={c.color} />
                <text x={22} y={y + 4} className="text-xs fill-zinc-700 font-medium">
                  {c.name}
                </text>
              </g>
            );
